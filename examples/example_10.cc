/**
 * @file example_10.cc
 * @brief Example 10: SimpleQueueDisc with weighted round-robin scheduling
 */
...
NS_LOG_COMPONENT_DEFINE("Example_10");
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("SimpleQueueDisc", LOG_LEVEL_DEBUG);
    ...
    // Uninstall existing queue discs from the NetDevices
    TrafficControlHelper tchUninstall;
    tchUninstall.Uninstall(d02);
    tchUninstall.Uninstall(d01);
    tchUninstall.Uninstall(d14);
    tchUninstall.Uninstall(d24);
    tchUninstall.Uninstall(d23);

    // Configure custom queuing on the switches
    TrafficControlHelper tch;
    tch.SetRootQueueDisc("ns3::SimpleQueueDisc");

    std::vector<std::pair<std::string, Ptr<SimpleQueueDisc>>> queueDiscs;

    QueueDiscContainer qd01 = tch.Install(d01);
    QueueDiscContainer qd02 = tch.Install(d02);
    QueueDiscContainer qd14 = tch.Install(d14);
    QueueDiscContainer qd24 = tch.Install(d24);
    QueueDiscContainer qd23 = tch.Install(d23);

    Ptr<SimpleQueueDisc> qd01_0 = DynamicCast<SimpleQueueDisc>(qd01.Get(0));
    Ptr<SimpleQueueDisc> qd02_0 = DynamicCast<SimpleQueueDisc>(qd02.Get(0));
    Ptr<SimpleQueueDisc> qd14_0 = DynamicCast<SimpleQueueDisc>(qd14.Get(0));
    Ptr<SimpleQueueDisc> qd24_0 = DynamicCast<SimpleQueueDisc>(qd24.Get(0));
    Ptr<SimpleQueueDisc> qd23_0 = DynamicCast<SimpleQueueDisc>(qd23.Get(0));

    queueDiscs.emplace_back("n0-n1", qd01_0);
    queueDiscs.emplace_back("n0-n2", qd02_0);
    queueDiscs.emplace_back("n1-n4", qd14_0);
    queueDiscs.emplace_back("n2-n4", qd24_0);
    queueDiscs.emplace_back("n2-n3", qd23_0);
    ...
    // trafficHelper.SetMaxPackets(5);
    trafficHelper.SetNumSlices(10);
    ...
    for (const auto& entry : queueDiscs)
    {
        std::cout << "Queue Statistics: " << entry.first << std::endl;
        entry.second->PrintQueueStatistics();
    }
